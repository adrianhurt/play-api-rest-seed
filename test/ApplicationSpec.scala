...
class ApplicationSpec extends PlaySpecification with JsonMatchers {
  ...
  def getRoute[A](method: String, uri: String, body: A, headers: Headers)(implicit w: Writeable[A]) = route(FakeRequest(method, uri, headers, body)).get
  ...
  "API" should {
    ...
    "render correctly the test page" in new WithApplication {
    ...
    "respond in the requested language" in new WithApplication {
    ...
    "not respond to unauthorized requests" in new WithApplication {
      mustBeError(ERROR_TOKEN_HEADER_NOTFOUND, routeGET("/account"))
    }
    "sign in" in new WithApplication {
      val result = routePOST("/signin", Json.obj("email" -> "user1@mail.com", "password" -> "123456"))
      status(result) must equalTo(OK)
      val response = Json.parse(contentAsString(result))
    ...
    "respond to authorized requests" in new WithApplication {
      signIn.map { token =>
    ...
    "paginate correctly" in new WithApplication {
      signIn.map { token =>
